...
# ══════════════════════════════════════════════════════════════
    ...
    if True:
        ...
        # ── CONTADOR PÚBLICO ──────────────────────────────────────────────────
        try:
            _pd  = qry("SELECT COUNT(*) AS n FROM contribuicoes_pendentes WHERE status='pendente'")
            _pa  = qry("SELECT COUNT(*) AS n FROM contribuicoes_pendentes WHERE status='aprovado'")
            n_pend  = int(_pd["n"].iloc[0])  if not _pd.empty  else 0
            n_aprov = int(_pa["n"].iloc[0]) if not _pa.empty else 0
            st.markdown("---")
