...
║                                                              ║
║     Requer: .streamlit/secrets.toml com credenciais          ║
║     do Supabase (veja secrets.toml.example)                  ║
║                                                              ║
║     Para publicar: GitHub → Streamlit Cloud                  ║
║       Secrets: cole o conteúdo do secrets.toml.example       ║
╚══════════════════════════════════════════════════════════════╝
"""
...
import httpx  # leve, zero dependências C++ — substitui o SDK supabase
...
st.set_page_config(
    ...
    menu_items={
        "Get Help": "https://github.com/MtzRJ",
        "Report a bug": "https://github.com/MtzRJ/observatorio-corrupcao-brasil/issues",
        "About": "Observatório da Corrupção Brasileira — desenvolvido por Matheus F. Valentim (github.com/MtzRJ). Banco educacional e de código aberto. Fontes: MPF, STF, TCU, Portal da Transparência, PF."
    }
)
...
# ── Conexão Supabase via httpx (sem dependências C++) ────────────────────────
# Usa a API REST do Supabase diretamente com httpx — funciona em qualquer OS
# sem precisar compilar nada. Zero dependências de build tools.

@st.cache_resource
def _get_creds():
    """Lê as credenciais do secrets.toml ou variáveis de ambiente."""
    try:
        ...
        st.error(
            ...
            "**Streamlit Cloud:** Settings → Secrets → adicione:\n"
...
def qry(sql: str) -> pd.DataFrame:
    """Executa SQL via RPC exec_sql e retorna DataFrame. Cache de 5 min."""
    url, _ = _get_creds()
    try:
        r = httpx.post(
        ...
        st.warning(f"Erro na query ({e.response.status_code}): {sql[:60]}...")
        return pd.DataFrame()
...
class _SB:
    """Mini-cliente Supabase usando httpx puro — sem SDK, sem C++."""
...
class _Table:
    ...
    def insert(self, dados: dict):
        return _Insert(self.nome, dados)


class _Insert:
    def __init__(self, tabela: str, dados: dict):
        self.tabela = tabela
        self.dados  = dados
...
# ── E-mail via Resend ─────────────────────────────────────────────────────────
def _resend_key() -> str:
    """Lê a chave do Resend dos secrets."""
    try:
...
def enviar_email_admin(tipo: str, nome_contrib: str, dados: dict, fonte: str):
    """
    Notifica o administrador quando chega uma nova contribuição.
    Disparado automaticamente após o formulário ser enviado.
    """
    if not RESEND_OK:
        return
    key = _resend_key()
    ...
    try:
        _resend.api_key = key
        titulo = dados.get("nome") or dados.get("razao_social") or                  f"{dados.get('pessoa_a_nome','')} → {dados.get('pessoa_b_nome','')}"
        contrib_label = nome_contrib or "Anônimo"

        html = f"""
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
          <div style="background:#0f1923;padding:24px;border-radius:8px 8px 0 0">
            <h2 style="color:#e8b84b;margin:0">🔍 Nova contribuição recebida</h2>
          </div>
          <div style="background:#f8f9fa;padding:24px;border:1px solid #dee2e6;border-radius:0 0 8px 8px">
            <table style="width:100%;border-collapse:collapse">
              <tr><td style="padding:8px 0;color:#666;width:140px"><strong>Tipo</strong></td>
                  <td style="padding:8px 0">{tipo.upper()}</td></tr>
              <tr><td style="padding:8px 0;color:#666"><strong>Título</strong></td>
                  <td style="padding:8px 0">{titulo}</td></tr>
              <tr><td style="padding:8px 0;color:#666"><strong>Contribuidor</strong></td>
                  <td style="padding:8px 0">{contrib_label}</td></tr>
              <tr><td style="padding:8px 0;color:#666"><strong>Fonte</strong></td>
                  <td style="padding:8px 0"><a href="{fonte}">{fonte[:60]}...</a></td></tr>
            </table>
            <div style="margin-top:20px;padding:12px;background:#fff3cd;border-radius:6px;
                        border-left:4px solid #e8b84b">
              <strong>Ação necessária:</strong> Acesse o Supabase → Table Editor →
              contribuicoes_pendentes e mude o status para <code>aprovado</code> ou
              <code>rejeitado</code>.
            </div>
          </div>
        </div>
        ...
        _resend.Emails.send({
            "from":    "Observatório <onboarding@resend.dev>",
            "to":      ["mtzcrpt@gmail.com"],
            "subject": f"[Observatório] Nova contribuição: {titulo[:50]}",
            "html":    html,
        })
    except Exception:
        pass  # e-mail é melhor esforço — não quebra o fluxo principal


def enviar_email_contribuidor(email_dest: str, nome: str, status: str,
                               titulo: str, nota: str = ""):
    """
    Notifica o colaborador sobre a decisão (aprovado ou rejeitado).
    Chamado pelo webhook do Supabase — ver instruções no README.
    """
    if not RESEND_OK or not email_dest:
        return
    key = _resend_key()
    if not key:
        return
    ...
    try:
        _resend.api_key = key
        aprovado = status.lower() == "aprovado"
        cor     = "#28a745" if aprovado else "#dc3545"
        emoji   = "✅" if aprovado else "❌"
        titulo_email = f"{emoji} Sua contribuição foi {'aprovada' if aprovado else 'rejeitada'}"

        html = f"""
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
          <div style="background:#0f1923;padding:24px;border-radius:8px 8px 0 0">
            <h2 style="color:#e8b84b;margin:0">🔍 Observatório da Corrupção Brasileira</h2>
          </div>
          <div style="background:#f8f9fa;padding:24px;border:1px solid #dee2e6;border-radius:0 0 8px 8px">
            <p>Olá{' ' + nome if nome else ''},</p>
            <p>Sua contribuição <strong>"{titulo}"</strong> foi analisada:</p>
            <div style="padding:16px;background:{cor}15;border-left:4px solid {cor};
                        border-radius:4px;margin:16px 0">
              <strong style="color:{cor}">{titulo_email}</strong>
              {"<p style='margin:8px 0 0'>" + nota + "</p>" if nota else ""}
            </div>
            {"<p>Os dados já estão disponíveis no banco público.</p>" if aprovado else
             "<p>Se tiver dúvidas ou quiser enviar uma nova contribuição com fonte adicional, acesse o site.</p>"}
            <hr style="border:none;border-top:1px solid #dee2e6;margin:20px 0">
            <p style="color:#666;font-size:12px">
              Observatório da Corrupção Brasileira · desenvolvido por
              <a href="https://github.com/MtzRJ">Matheus F. Valentim</a>
            </p>
          </div>
        </div>
        """
...
# ── Dados carregados via Supabase ─────────────────────────────────────────────
df_casos    = qry("SELECT * FROM casos ORDER BY valor_reais DESC NULLS LAST")
df_pessoas  = qry("SELECT * FROM pessoas ORDER BY nome")
df_empresas = qry("SELECT * FROM empresas")
df_partidos = qry("SELECT * FROM partidos")
df_ops      = qry("SELECT * FROM operacoes")

# views
df_resumo   = qry("SELECT * FROM v_casos_resumo")
df_rede     = qry("SELECT * FROM v_rede_relacionamentos")
df_part_rank= qry("SELECT * FROM v_partidos_escandalo ORDER BY total_politicos_investigados DESC")
...
with st.sidebar:
    ...
    <hr style="border-color:#1a2940; margin: 8px 0">
    <span style="color:#6a8aaa">Desenvolvido por</span><br>
    <strong style="color:#8ba3c0">Matheus F. Valentim</strong><br>
    <a href="https://github.com/MtzRJ" target="_blank"
       style="color:#2e7ab8; text-decoration:none">⌥ github.com/MtzRJ</a>
    </div>
    """, unsafe_allow_html=True)


# ── Filtrar por anos ──────────────────────────────────────────────────────────
# Proteção: se o banco ainda não foi populado, df pode vir vazio
if df_casos.empty or "ano_inicio" not in df_casos.columns:
    df_casos_f = df_casos
else:
    df_casos_f = df_casos[
...
if df_pessoas.empty or "tipo" not in df_pessoas.columns:
    df_pessoas_f = df_pessoas
else:
...
<div class="main-header">
  ...
  <p class="main-subtitle">
    Banco educacional e open source · 1988–presente · Fontes: MPF, STF, TCU, CGU, PF
  </p>
...
if pagina == "🏠 Visão Geral":
    ...
    st.markdown("""
    <div class="warn-box">
    ⚠️ <strong>Nota metodológica:</strong> Valores são estimativas baseadas em relatórios do MPF, TCU, COAF e acordos de colaboração premiada. Investigações em andamento podem ter novos desdobramentos. Este banco tem fins educacionais e não substitui a consulta às fontes primárias.
    </div>
    """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════
    ...
    order_map = {
        "Valor (maior primeiro)":    ("valor_reais", False),
        "Valor (menor primeiro)":    ("valor_reais", True),
        "Ano (mais recente primeiro)": ("ano_inicio", False),
        "Ano (mais antigo primeiro)":  ("ano_inicio", True),
    }
...
# ══════════════════════════════════════════════════════════════
    ...
    with col_p2:
        tipo_filtro = st.multiselect("Filtrar por tipo", df_pessoas["tipo"].unique().tolist(), default=df_pessoas["tipo"].unique().tolist())
    with col_p3:
    ...
    for _, row in df_p.iterrows():
        ...
        with st.expander(f"{icon} **{row['nome']}** · {row['tipo']} · {row['partido'] or '—'}"):
            ...
            df_rel_p = qry(f"""
                ...
                WHERE r.pessoa_a = {int(row["id"])} OR r.pessoa_b = {int(row["id"])}
            """)
    ...
    for _, row in df_e.iterrows():
        ...
        with st.expander(f"{icon} **{row['razao_social']}** · {row['tipo']} · {row['pais']}"):
            ...
            df_casos_e = qry(f"""
                ...
                WHERE ce.empresa_id = {int(row["id"])}
            """)
            ...
            df_socios = qry(f"""
                ...
                WHERE pe.empresa_id = {int(row["id"])}
            """)
...
# ══════════════════════════════════════════════════════════════
    ...
    st.markdown("---")
    st.markdown("#### 💡 Por que o PMDB/MDB aparece em tantos escândalos{obs_p}")
    st.markdown("""
    <div class="info-box">
...
# ══════════════════════════════════════════════════════════════
    ...
    if nome_sel:
        ...
        df_rede_p = qry(f"""
            ...
            JOIN pessoas pb ON pb.id = r.pessoa_b
            LEFT JOIN casos c ON c.id = r.caso_id
            WHERE r.pessoa_a = {int(pessoa_id)} OR r.pessoa_b = {int(pessoa_id)}
        """)
    ...
    if st.button("📥 Exportar rede completa para análise externa (Gephi / D3.js)"):
        ...
        rels_raw = qry("SELECT * FROM relacionamentos")
...
# ══════════════════════════════════════════════════════════════
    ...
    if True:
        ...
        with st.expander("👤 Sua identificação (opcional)", expanded=False):
        ...
        if "Novo caso" in tipo_contribuicao:
            with st.form("contrib_caso"):
                ...
                operac = st.text_input("Nome da operação policial (se houver)", placeholder="Ex: Lava Jato, Carne Fraca...")
                ...
                pessoas_env = st.text_area("Pessoas envolvidas (uma por linha)",
                    placeholder="Ex:\nFulano de Tal — ex-governador SP\nSiclano Silva — doleiro",
                    height=90,
                    help="Liste os principais investigados. Eles serão adicionados manualmente após a revisão.")
                ...
                if enviado:
                    if not nome_c or not desc or not fonte_url or not confirmado:
                        ...
                        dados_para_salvar = {
                            ...
                            "pessoas_envolvidas": pessoas_env or "",
                        }
                        form_ok = True
            ...
            df_p_contrib = qry("SELECT nome FROM pessoas ORDER BY nome")
            ...
            with st.form("contrib_rel"):
                ...
                casos_lista = qry("SELECT nome FROM casos ORDER BY nome")["nome"].tolist()
        ...
        if form_ok and dados_para_salvar:
            # Mapeamento explícito — evita depender do split() do label com emoji
            _TIPO_MAP = {
                "🗂️ Novo caso ou escândalo":          "caso",
                "👤 Nova pessoa investigada":           "pessoa",
                "🔗 Novo relacionamento entre pessoas": "relacionamento",
                "🏢 Nova empresa investigada":          "empresa",
            }
            tipo_str = _TIPO_MAP.get(tipo_contribuicao, "outro")
            try:
                sb.table("contribuicoes_pendentes").insert({
                    "tipo":               tipo_str,
                    "nome_contribuidor":  contrib_nome or "",
                    "email_contribuidor": contrib_email or "",
                    "dados_json":         dados_para_salvar,
                    "fonte_url":          fonte_url or "",
                    "fonte_descricao":    fonte_desc or "",
                }).execute()
                # Notificar admin por e-mail
                enviar_email_admin(
                    tipo=tipo_str,
                    nome_contrib=contrib_nome,
                    dados=dados_para_salvar,
                    fonte=fonte_url or ""
                )
                st.success("✅ Contribuição enviada! Será analisada em breve.")
                st.info("📬 Obrigado! Após verificação das fontes, os dados entram no banco. Se informou seu e-mail, receberá uma resposta.")
                st.cache_data.clear()
            except Exception as e:
        ...
        # ── CONTADOR PÚBLICO ──────────────────────────────────────────────────
        try:
            _pd  = qry("SELECT COUNT(*) AS n FROM contribuicoes_pendentes WHERE LOWER(status)='pendente'")
            _pa  = qry("SELECT COUNT(*) AS n FROM contribuicoes_pendentes WHERE LOWER(status)='aprovado'")
            n_pend  = int(_pd["n"].iloc[0])  if not _pd.empty and "n" in _pd.columns else 0
            n_aprov = int(_pa["n"].iloc[0]) if not _pa.empty and "n" in _pa.columns else 0
            st.markdown("---")
...
# ══════════════════════════════════════════════════════════════
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["🎯 O que é", "🎓 Para pesquisadores", "💻 Desenvolvedores", "📊 Expandir o banco"])
    ...
    st.markdown("---")
    st.markdown("### 🗄️ Banco de dados completo")
    st.markdown("""
    <div class="warn-box">
    ⚠️ <strong>Para administradores:</strong> baixe o banco abaixo após aprovar contribuições
    e envie o arquivo de volta ao GitHub para que as alterações fiquem permanentes no site.
    </div>
    """, unsafe_allow_html=True)
    st.markdown("""
    <div class="info-box">
    💡 Com o Supabase, o banco vive na nuvem. Para exportar dados, use o painel
    do Supabase em <strong>app.supabase.com → Table Editor → Export</strong>,
    ou baixe os CSVs acima.
    </div>
    """, unsafe_allow_html=True)
